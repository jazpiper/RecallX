...
| Agent work update | implementation summary or completed task note | activity | No | Preferred default for finished work |
...
| Agent decision    | decision node          | suggested             | Yes by default  | Non-trusted agent decisions stay in review |
| Import            | node                   | imported              | No              | - |
| Claude Code / Codex | run summary          | activity              | No              | - |
| Trusted source tool | technical decision   | appended active node  | No              | Applies when `toolName` is listed in `review.trustedSourceToolNames` |
| Claude Code / Codex | technical decision   | suggested note by default | Yes         | Trusted-source settings can bypass review |
...
## Core principles
- Decision nodes require human approval by default, unless the source tool is explicitly trusted in settings  
...
Trusted-source settings are the explicit policy exception in v1.
